...
hesap= {"ad": "Selen Yalın", "hesapNumarası": "3456237", "bakiye": 35785, "ekHesap": 2000}
...
def para_yatirma(hesap):
    miktar= float(input("Lütfen yatırmak istediğiniz tutarı yazınız: "))
    hesap["bakiye"] += miktar
    print(f"{miktar} TL başarıyla yatırılmıştır. Güncel bakiye: {hesap["bakiye"]}")
...
def para_cekme(hesap):
    ...
    if miktar <= hesap["bakiye"]:
        hesap["bakiye"] -= miktar
        print(f"{miktar} TL nizi alabilirsiniz. Güncel bakiye: {hesap["bakiye"]} TL")
    else:
